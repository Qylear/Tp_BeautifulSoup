...
def fetch_article_content(url: str) -> Tuple[Optional[str], Dict[str, str], Optional[str]]:
    try:
        ...
        return clean_text(content), images, author
    except Exception as e:
    ...
    try:
        ...
        for index, article in enumerate(articles, 1):
            try:
                ...
                if article_data:
                    content, images, author = fetch_article_content(article_data['url'])
                    article_data.update({
                        'content': content,
                        'images': images,
                        'author': author,
...
def extract_thumbnail(article: BeautifulSoup) -> Optional[str]:
    ...
    if img_tag:
        return img_tag.get('data-lazy-src') or img_tag.get('src')
    return None
